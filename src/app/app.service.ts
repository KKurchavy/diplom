...
export class AppService {
  ...
  private _isEngRus: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    true
  );
  private _controlMode: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  ); //false - education, true - control
  private _splitMode: BehaviorSubject<string> = new BehaviorSubject<string>("");
  private _loadingWindow: BehaviorSubject<boolean> = new BehaviorSubject<
    boolean
  >(false);
  private _allPermissions: BehaviorSubject<boolean> = new BehaviorSubject<
    boolean
  >(true);
  ...
  // get controlWords(): Observable<Word[]> {
  //   return this.http
  //     .get<WordResponse>(`${this.url}/words`)
  //     .map(({ words }) => words.map(item => ({ ...item, done: false })));
  // }
  get controlWords(): Observable<Word[]> {
    return new BehaviorSubject<Word[]>(words);
  }
  ...
  public sendStatistic(student: Student, value: any): void {
    this.http
      .post(`${this.url}/statistics/5adc52ae9e41e9112c3c1de3`, {
        rofl: "tut bil yan"
      })
      .subscribe(d => console.log(d));
    this.http.get(`${this.url}/statistics`).subscribe(d => console.log(d));
    console.log(student);
  }
  ...
  public loginAdmin(user: Admin): Observable<any> {
    ...
    /* return new BehaviorSubject({
      token: "типо токен, но на самом деле нет : ) "
